...
@section('content')
    <div class="mx-auto max-w-4xl space-y-6">
        <div>
            <h1 class="section-title">Editar partido</h1>
            <p class="section-subtitle">Actualiza datos, estado y resultado del encuentro.</p>
        </div>

        <x-card>
            <form method="POST" action="{{ route('admin.match-games.update', $matchGame) }}" class="space-y-4">
                @csrf
                @method('PUT')
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    <div>
                        ...
                        <input name="phase" value="{{ old('phase', $matchGame->phase) }}" class="w-full rounded-lg border border-slate-600 bg-slate-900 px-3 py-2 text-slate-100" required>
                    ...
                    <div>
                        ...
                        <input name="group_name" value="{{ old('group_name', $matchGame->group_name) }}" class="w-full rounded-lg border border-slate-600 bg-slate-900 px-3 py-2 text-slate-100">
                    ...
                    <div>
                        ...
                        <select name="status" class="w-full rounded-lg border border-slate-600 bg-slate-900 px-3 py-2 text-slate-100" required>
                            @foreach (['scheduled', 'live', 'finished', 'cancelled'] as $status)
                                <option value="{{ $status }}" @selected(old('status', $matchGame->status) === $status)>{{ $status }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" min="0" max="30" name="home_score" value="{{ old('home_score', $matchGame->home_score) }}" class="w-full rounded-lg border border-slate-600 bg-slate-900 px-3 py-2 text-slate-100">
                    ...
                    <div>
                        ...
                        <input type="number" min="0" max="30" name="away_score" value="{{ old('away_score', $matchGame->away_score) }}" class="w-full rounded-lg border border-slate-600 bg-slate-900 px-3 py-2 text-slate-100">
                        @error('away_score') <p class="mt-1 text-xs text-red-300">{{ $message }}</p> @enderror
                    </div>
                </div>

                <div class="flex items-center gap-3 pt-2">
                    <x-button type="submit">Actualizar partido</x-button>
                    <a href="{{ route('admin.match-games.index') }}" class="rounded-lg bg-slate-700 px-4 py-2 text-sm font-semibold text-slate-100 hover:bg-slate-600">Volver</a>
                </div>
            </form>
        </x-card>

        <x-card title="Carga de resultados" subtitle="Este bloque recalcula puntos con ScoringService al guardar.">
            <form method="POST" action="{{ route('admin.match-games.update-result', $matchGame) }}" class="grid gap-4 sm:grid-cols-3 sm:items-end">
                @csrf
                @method('PATCH')

                <div>
                    <label class="mb-1 block text-sm font-semibold text-slate-200">Goles local</label>
                    <input type="number" min="0" max="30" name="home_score" value="{{ old('home_score', $matchGame->home_score) }}" class="w-full rounded-lg border border-slate-600 bg-slate-900 px-3 py-2 text-slate-100" required>
                </div>
                <div>
                    <label class="mb-1 block text-sm font-semibold text-slate-200">Goles visitante</label>
                    <input type="number" min="0" max="30" name="away_score" value="{{ old('away_score', $matchGame->away_score) }}" class="w-full rounded-lg border border-slate-600 bg-slate-900 px-3 py-2 text-slate-100" required>
                </div>
                <input type="hidden" name="status" value="finished">
                <x-button type="submit" variant="success" class="sm:w-auto">Guardar resultado final</x-button>
            </form>
        </x-card>
    </div>
